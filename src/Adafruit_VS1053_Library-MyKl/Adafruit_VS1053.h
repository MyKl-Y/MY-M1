...
public:
  ...
  /** 
   * @brief Seconds played so far (from VS1053 internal timer)
   */
  uint16_t getElapsedTime(void);

  /**
   * @brief Estimate total track length in seconds 
   *        (fileSize−ID3Offset) * elapsed / bytesDecoded
   */
  uint16_t getTotalTime(void);

  /**
   * @brief Jump to approx. `seconds` into the current track
   */
  void     seekTime(uint16_t seconds);
