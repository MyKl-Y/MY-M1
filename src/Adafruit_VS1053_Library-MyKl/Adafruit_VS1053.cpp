...
boolean Adafruit_VS1053_FilePlayer::startPlayingFile(const char *trackname) {
  ...
  unsigned long id3len = 0;
  if (isMP3File(trackname)) {
    id3len = mp3_ID3Jumper(currentTrack);
    currentTrack.seek(id3len);
  }
  _id3Offset = id3len;
...
// ----------------------------------------------------------------------------
// Return how many seconds have been decoded/playbacked so far:
uint16_t Adafruit_VS1053_FilePlayer::getElapsedTime() {
  // VS1053 chip keeps its own seconds counter in DECODETIME
  return Adafruit_VS1053::decodeTime();
}

// ----------------------------------------------------------------------------
// Estimate total duration in seconds, by scaling fileSize vs bytesDecoded:
uint16_t Adafruit_VS1053_FilePlayer::getTotalTime() {
  uint32_t fileSize   = currentTrack.size();
  uint32_t decodedPos = currentTrack.position() - _id3Offset;
  uint16_t elapsed    = getElapsedTime();

  if (decodedPos == 0) return 0;    // avoid divide-by-zero
  uint32_t dataLen    = fileSize - _id3Offset;

  // scale up: total ≈ dataLen * elapsed / decodedPos
  return (uint16_t)( (uint64_t)dataLen * elapsed / decodedPos );
}

// ----------------------------------------------------------------------------
// Seek to 'seconds' into the track (approximate):
void Adafruit_VS1053_FilePlayer::seekTime(uint16_t seconds) {
  uint16_t total = getTotalTime();
  ...
  // compute byte offset in the "audio data" region
  uint32_t target = (uint64_t)dataLen * seconds / total;
  // jump to that spot in the SD card file
  currentTrack.seek(_id3Offset + target);
  // refill chip buffer immediately
  feedBuffer();
}
